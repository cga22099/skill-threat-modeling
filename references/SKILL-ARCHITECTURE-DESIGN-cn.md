...
│  │  │                                                                                    │ │
│  │  ├── list_files.py (11KB)            ◄── Phase 1: 项目结构                           │ │
│  │  │   └── 文件分类, 类型检测                                                          │ │
│  │  │                                                                                    │ │
│  │  ├── stride_matrix.py (7.6KB)        ◄── Phase 5: STRIDE per Interaction            │ │
│  │  │   └── 威胁类别计算                                                                │ │
│  │  │                                                                                    │ │
│  │  ├── collect_code_stats.py (18KB)    ◄── Phase 1: LOC/文件统计                       │ │
│  │  │   └── 语言分布, 模块识别                                                          │ │
│  │  │                                                                                    │ │
│  │  └── validate_count_conservation.py  ◄── 质量保证                                    │ │
│  │      └── P5→P6 计数守恒验证                                                          │ │
│  │                                                                                        │ │
│  └───────────────────────────────────────────────────────────────────────────────────────┘ │
│                                              │                                              │
...
│  ║                                                                                        ║ │
│  ║  步骤:                                          脚本:                                  ║ │
│  ║  1. 获取文件结构 ────────────────────────────► list_files.py --categorize             ║ │
│  ║  2. 识别项目类型                                                                       ║ │
│  ║  3. 收集规模指标 ─────────────────────────────► collect_code_stats.py                 ║ │
│  ║  4. 读取关键文件                                                                       ║ │
│  ║  5. 记录架构                                                                           ║ │
│  ║                                                                                        ║ │
...
│  ║  5. 验证计数守恒 ──────────────────────────────► validate_count_conservation.py      ║ │
│  ║                                                                                        ║ │
...
| 阶段 | 命令 | 用途 |
|-------|---------|---------|
| P1 | `python list_files.py <path> --categorize --detect-type` | 获取项目结构 |
| P1 | `python collect_code_stats.py <path> --output json` | 收集 LOC 指标 |
...
| P8 | `python validate_count_conservation.py ./Risk_Assessment_Report/` | 验证计数 |
...
| 威胁 Threat | `T-{STRIDE}-{Element}-{Seq}` | T-T-P13-002, T-S-DS01-001 | P5 |
